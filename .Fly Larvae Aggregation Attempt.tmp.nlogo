...
to create-food-wall ;; patch setup
  ask patches [
    ...
    set food? false
    if food-present? [
      if distancexy 10 0 < 5 [ ;; circular food source. Arbitrary size
        set food? true
        set pcolor red
      ]
    ]
...
to setup-vectors
  if vectors? [
    ...
    set vector-scale 1
    ask patches [
      if pxcor mod 3 = 0 and pycor mod 3 = 0 and not (wall? or food?) [
        sprout-vectors 1 [
...
to avoid-obstacle [dist] ;; helper method for move
  if is-obstacle? patch-ahead dist [
    ...
    while [ (not (can-turn-r? or can-turn-l?)) and (i * increment <= 180) ] [
      set i ( i + 1 )
...
to-report is-obstacle? [ target ] ;; helper method for avoid-obstacle
  ; if (volume-exclusion? and larvae-ahead?) [ show (not is-patch? target) or ([ wall? or food? ] of target) or (volume-exclusion? and larvae-ahead?) ]
  ;if not is-patch? target [ report true ]
  ;report  ([ wall? or food? ] of target) or (volume-exclusion? and (larvae-ahead? target))
...
to-report larvae-ahead? [target]
  ifelse (count other larvae-on target) >= max-larvae-patch [
    report true ]
  [ report false ]
end
...
to eat ;; larva procedure
  if (is-patch? patch-ahead vision) and [food?] of patch-ahead vision [
    set eating-time-left random-normal 10 20 ;; experimentally determined
...
## WHAT IS IT? (only this is updated)

This model is an attempt to mimic the behavior of the larvae aggregations of the Black Soldier Fly (_Hermetia illucens_). Larvae move in a petri dish world of gray boundaries with red food present. A larvae's movement is based on a model for individual larva fitted to physical experiments.
